...
func TestBuildControllerUnexpectedContainerID(t *testing.T) {
	f := newTestFixture(t)
	defer f.TearDown()
	...
	// so there is no build in which a container build can happen, so this buildstate should not have any deployinfo
	assert.True(t, call.oneState().DeployInfo.Empty())
